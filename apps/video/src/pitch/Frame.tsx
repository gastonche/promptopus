...
export const CinematicBackground: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: C.inkDeep }}>
      <AbsoluteFill
        style={{ background: `radial-gradient(38% 55% at ${g1x}% 28%, ${C.pop700}55 0%, transparent 70%)`, opacity: pulse }}
      />
      <AbsoluteFill
        style={{ background: `radial-gradient(36% 52% at ${g2x}% 78%, ${C.pop500}44 0%, transparent 70%)`, opacity: 1.6 - pulse }}
      />
      <AbsoluteFill style={{ background: `radial-gradient(130% 130% at 50% 50%, transparent 50%, ${C.inkDeep}dd 100%)` }} />
    </AbsoluteFill>
  );
};
...
export const Letterbox: React.FC = () => {
  const frame = useCurrentFrame();
  const h = interpolate(frame, [0, 18], [140, 88], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  return (
...
export const FadeScene: React.FC<{ dur: number; children: ReactNode }> = ({ dur, children }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity }}>
      <AbsoluteFill style={{ transform: `scale(${scale})`, alignItems: 'center', justifyContent: 'center' }}>
        {children}
      </AbsoluteFill>
    </AbsoluteFill>
  );
