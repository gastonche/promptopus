...
const Stack: React.FC<{ children: React.ReactNode; gap?: number }> = ({ children, gap = 0 }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap, fontFamily: sans }}>
    {children}
  </div>
);
...
export const ColdOpen: React.FC = () => {
  ...
  return (
    <Stack gap={30}>
      <div style={{ fontFamily: mono, fontSize: 30, letterSpacing: 4, textTransform: 'uppercase', color: C.pop300, opacity: k }}>
        shipping an llm feature?
      </div>
      <div style={{ ...big, fontSize: 92, fontWeight: 800, color: '#fff', letterSpacing: -2, textAlign: 'center' }}>
      ...
      <div style={{ ...verdict, fontSize: 40, fontWeight: 700, color: K }}>That's not an evaluation.</div>
    </Stack>
  );
};
  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', fontFamily: sans }}>
      {PROVIDERS.map((p, i) => {
        ...
        const appear = interpolate(frame, [10 + i * 4, 26 + i * 4], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
        const pull = interpolate(frame, [44, 72], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
        const R = 360 * pull;
...
const TwoCol: React.FC<{ left: React.ReactNode; right: React.ReactNode }> = ({ left, right }) => (
  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center', width: 1600, fontFamily: sans }}>
    <div>{left}</div>
    <div>{right}</div>
  </div>
);

const Heading: React.FC<{ kicker: string; title: React.ReactNode; sub?: string }> = ({ kicker, title, sub }) => {
  const frame = useCurrentFrame();
  const a = fadeUp(frame, 6, 16, 28);
  const b = fadeUp(frame, 16, 16, 24);
  return (
    <div style={{ ...a }}>
      <div style={{ fontFamily: mono, fontSize: 26, letterSpacing: 4, textTransform: 'uppercase', color: C.pop500, fontWeight: 700 }}>
        {kicker}
      </div>
      <div style={{ marginTop: 14, fontSize: 76, fontWeight: 900, color: '#fff', letterSpacing: -2, lineHeight: 1.05 }}>
        {title}
      </div>
      {sub && <div style={{ ...b, marginTop: 18, fontSize: 32, color: '#cdc7e8', lineHeight: 1.4 }}>{sub}</div>}
    </div>
  );
};
...
export const Define: React.FC = () => {
  ...
  return (
    <TwoCol
      left={<Heading kicker="01 · Define" title={<>Define it<br />once.</>} sub="One YAML file: your cases, your models, your graders." />}
      right={
...
const RUN_MODELS = ['gpt-4o-mini', 'llama-3.1-8b', 'mistral-24b', 'gemma-12b', 'qwen-32b', 'qwq-32b'];
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {RUN_CASES.map((c, row) => (
          <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 220, textAlign: 'right', fontFamily: mono, fontSize: 22, color: '#bdb7d6' }}>{c}</div>
...
const COMPARE_ROWS: Row[] = [
  { label: 'Pass rate', vals: [{ v: 99, fmt: (n) => `${Math.round(n)}%`, tone: 'best' }, { v: 92, fmt: (n) => `${Math.round(n)}%` }, { v: 92, fmt: (n) => `${Math.round(n)}%` }, { v: 39, fmt: (n) => `${Math.round(n)}%`, tone: 'worst' }] },
  { label: 'Judge', vals: [{ v: 1.0, fmt: (n) => n.toFixed(2), tone: 'best' }, { v: 1.0, fmt: (n) => n.toFixed(2) }, { v: 0.93, fmt: (n) => n.toFixed(2) }, { v: 0.77, fmt: (n) => n.toFixed(2), tone: 'worst' }] },
  { label: 'Cost', vals: [{ v: 0.0003, fmt: (n) => `$${n.toFixed(4)}`, tone: 'best' }, { v: 0.0006, fmt: (n) => `$${n.toFixed(4)}` }, { v: 0.0021, fmt: (n) => `$${n.toFixed(4)}` }, { v: 0.0033, fmt: (n) => `$${n.toFixed(4)}`, tone: 'worst' }] },
  { label: 'p95 latency', vals: [{ v: 3220, fmt: (n) => `${Math.round(n)}ms`, tone: 'best' }, { v: 3859, fmt: (n) => `${Math.round(n)}ms` }, { v: 4185, fmt: (n) => `${Math.round(n)}ms` }, { v: 15837, fmt: (n) => `${Math.round(n)}ms`, tone: 'worst' }] },
];
...
export const Compare: React.FC = () => {
  ...
  return (
    <Stack gap={36}>
      ...
      <div style={{ width: 1500, borderRadius: 20, overflow: 'hidden', border: `1px solid ${C.pop900}`, background: '#1b1830', fontFamily: sans }}>
        <div style={{ display: 'flex', background: C.pop700, color: '#fff', padding: '18px 28px', fontSize: 28, fontWeight: 700 }}>
          <div style={{ flex: 1.3 }}>Metric</div>
          {COMPARE_MODELS.map((m) => (
            <div key={m} style={{ flex: 1, textAlign: 'right', fontSize: 24 }}>{m}</div>
          ))}
        </div>
        {COMPARE_ROWS.map((r, i) => {
          ...
          return (
            <div key={r.label} style={{ display: 'flex', alignItems: 'center', padding: '18px 28px', fontSize: 30, background: i % 2 ? '#ffffff08' : 'transparent', opacity: a.opacity, transform: a.transform }}>
              <div style={{ flex: 1.3, color: '#cdc7e8' }}>{r.label}</div>
              {r.vals.map((cell, ci) => (
                <div key={ci} style={{ flex: 1, textAlign: 'right', fontFamily: mono, fontWeight: 700, color: tone(cell.tone) }}>
...
// 6 — GRADER FAMILIES
const FAMS = [
  { tag: 'free · instant', name: 'Deterministic', accent: C.pass, icon: 'M20 6 9 17l-5-5', items: ['contains · regex', 'json-schema', '+ your own'] },
  { tag: 'faithfulness · quality', name: 'LLM-as-judge', accent: C.pop400, icon: 'M12 3v18M5 7h14M5 7l-3 7a4 4 0 0 0 6 0L5 7Zm14 0-3 7a4 4 0 0 0 6 0l-3-7Z', items: ['structured scores', 'graceful failure'] },
  { tag: 'budgets · p50/p95', name: 'Cost + latency', accent: C.warn, icon: 'M12 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm0 0 5-5M4 18a9 9 0 1 1 16 0', items: ['per-call budgets', 'first-class metrics'] },
];
    ...
    <Stack gap={48}>
      ...
      <div style={{ display: 'flex', gap: 28 }}>
        {FAMS.map((f, i) => {
          ...
          return (
            <div key={f.name} style={{ width: 420, background: '#1b1830', border: `1px solid ${f.accent}44`, borderRadius: 22, padding: 34, opacity: s, transform: `translateY(${y}px)`, fontFamily: sans, boxShadow: `0 30px 80px -30px ${f.accent}55` }}>
              <div style={{ width: 66, height: 66, borderRadius: 16, background: `${f.accent}22`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width={34} height={34} viewBox="0 0 24 24" fill="none" stroke={f.accent} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                  <path d={f.icon} />
                </svg>
              </div>
              <div style={{ marginTop: 20, color: f.accent, fontFamily: mono, fontSize: 22, fontWeight: 700 }}>{f.tag}</div>
              <div style={{ marginTop: 6, color: '#fff', fontSize: 38, fontWeight: 800 }}>{f.name}</div>
...
export const Proof: React.FC = () => {
  ...
  const Stat: React.FC<{ to: number; suffix?: string; label: string }> = ({ to, suffix = '', label }) => (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ fontFamily: mono, fontSize: 22, color: C.pop300, letterSpacing: 1 }}>{label}</div>
    </div>
  );
  ...
  return (
    <Stack gap={40}>
      <div style={{ ...t, textAlign: 'center' }}>
        <div style={{ fontFamily: mono, fontSize: 26, letterSpacing: 4, textTransform: 'uppercase', color: C.pop500, fontWeight: 700 }}>We dogfood it</div>
        <div style={{ marginTop: 12, fontSize: 72, fontWeight: 900, color: '#fff', letterSpacing: -2 }}>A real benchmark, 10 models deep.</div>
      </div>

      <div style={{ display: 'flex', gap: 80, opacity: counters.opacity, transform: counters.transform }}>
      ...
      <div style={{ display: 'flex', gap: 60, alignItems: 'stretch', marginTop: 4 }}>
        <div style={{ transform: `scale(${interpolate(win, [0, 1], [0.7, 1])})`, opacity: win, textAlign: 'center', padding: '24px 50px', borderRadius: 20, background: `${C.pass}14`, border: `1px solid ${C.pass}55` }}>
          <div style={{ fontSize: 140, fontWeight: 900, color: C.pass, lineHeight: 1, textShadow: `0 0 50px ${C.pass}66` }}>99%</div>
          <div style={{ marginTop: 10, fontSize: 28, color: '#fff', fontWeight: 700 }}>llama-3.1-8b</div>
          <div style={{ fontSize: 22, color: '#cdc7e8' }}>the open 8B model · $0.0003</div>
        </div>
        <div style={{ transform: `scale(${interpolate(lose, [0, 1], [0.7, 1])})`, opacity: lose, textAlign: 'center', padding: '24px 50px', borderRadius: 20, background: `${C.fail}14`, border: `1px solid ${C.fail}55` }}>
          <div style={{ fontSize: 140, fontWeight: 900, color: C.fail, lineHeight: 1, textShadow: `0 0 50px ${C.fail}66` }}>39%</div>
...
export const Extend: React.FC = () => {
  ...
  return (
    <TwoCol
      left={<Heading kicker="04 · Extend" title={<>In your<br />own code.</>} sub="Add a provider or grader in a promptopus.config.mjs — no fork." />}
      right={
...
export const CTA: React.FC = () => {
  ...
  return (
    <Stack gap={14}>
      ...
      <div style={{ ...tag, fontSize: 40, fontWeight: 600, color: '#cdc7e8' }}>Stop guessing which model to ship.</div>
      <div
        style={{
          marginTop: 26,
          padding: '20px 40px',
      ...
      <div style={{ ...links, marginTop: 24, display: 'flex', gap: 36, fontFamily: mono, fontSize: 28, color: '#bdb7d6' }}>
