...
const ROWS: Row[] = [
  {
    label: 'Pass rate',
    a: { text: (v) => `${Math.round(v)}%`, to: 100 },
    b: { text: (v) => `${Math.round(v)}%`, to: 100 },
  },
  {
    label: 'Judge score',
    a: { text: (v) => v.toFixed(2), to: 0.96, tone: 'worst' },
    b: { text: (v) => v.toFixed(2), to: 0.99, tone: 'best' },
  },
  {
    label: 'Cost · total',
    a: { text: (v) => `$${v.toFixed(4)}`, to: 0.0005, tone: 'worst' },
    b: { text: (v) => `$${v.toFixed(4)}`, to: 0.0002, tone: 'best' },
  },
  {
    label: 'Latency · p95',
    a: { text: (v) => `${Math.round(v)}ms`, to: 2436, tone: 'best' },
    b: { text: (v) => `${Math.round(v)}ms`, to: 6880, tone: 'worst' },
  },
];
