...
const SLAMS: Slam[] = [
  ...
  {
    big: '3',
    label: 'GRADER FAMILIES',
    sub: 'deterministic · judge · cost/latency',
    accent: C.warn,
  },
  {
    big: '4',
    label: 'PROVIDERS',
    sub: 'OpenAI · Anthropic · Workers AI · local',
    accent: C.pop300,
  },
];
...
const SlamView: React.FC<{ data: Slam; start: number }> = ({ data, start }) => {
  ...
  const outO = interpolate(local, [WINDOW - 12, WINDOW], [1, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const inO = interpolate(local, [0, 6], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const flash = interpolate(local, [0, 9], [0.5, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  ...
  return (
    <AbsoluteFill
      style={{ alignItems: 'center', justifyContent: 'center', fontFamily: sans, opacity }}
    >
      <AbsoluteFill
        style={{
          background: `radial-gradient(45% 45% at 50% 45%, ${data.accent}33 0%, transparent 70%)`,
          opacity,
        }}
      />
      <div style={{ transform: `scale(${scale})`, textAlign: 'center' }}>
        ...
        <div style={{ marginTop: 14, fontSize: 34, fontWeight: 600, color: '#cdc7e8' }}>
          {data.sub}
        </div>
      </div>
      <AbsoluteFill
        style={{ background: data.accent, opacity: flash * 0.25, pointerEvents: 'none' }}
      />
    </AbsoluteFill>
  );
};
