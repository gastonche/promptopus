...
const LINES: React.ReactNode[] = [
  <><Tok c={K}>import</Tok> {'{'} defineConfig {'}'} <Tok c={K}>from</Tok> <Tok c={S}>'promptopus'</Tok>;</>,
  <> </>,
...
export const Extend: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', fontFamily: sans, padding: 70 }}>
      <div style={{ textAlign: 'center', opacity: t.opacity, transform: t.transform, marginBottom: 40 }}>
        ...
        <div style={{ marginTop: 12, fontSize: 36, fontWeight: 600, color: '#cdc7e8' }}>
          No fork. Add a provider or grader in a <span style={{ color: C.pop300 }}>promptopus.config.mjs</span>
        </div>
      </div>
      <CodeCard title="promptopus.config.mjs" fontSize={26} style={{ width: 968, opacity: s, transform: `translateX(${x}px)` }}>
        {LINES.map((line, i) => {
