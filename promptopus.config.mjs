...
const det = (id, score, passed, detail) => ({
  graderId: id,
...
export default defineConfig({
  graders: {
    ...
    'no-meta-reference': () => {
      const re =
        /\b(this (?:article|text|content|passage|piece|summary)|the (?:article|text|author|passage|content)|in summary|to summari[sz]e)\b/i;
      return {
        ...
        grade: ({ output }) => {
          ...
          return det(
            ...
            m ? 0 : 1,
            !m,
            m ? `meta-reference: "${m[0]}"` : 'no meta-references',
          );
        },
      };
    },
    ...
    'no-reasoning-leak': () => {
      const re =
        /<\/?think>|\b(chain[- ]of[- ]thought)\b|^\s*(?:okay,?\s+(?:so|let)|let me|first,?\s+i|i need to|i'?ll start|alright,)/i;
      return {
        ...
        grade: ({ output }) => {
          ...
          return det(
            ...
            bad ? 0 : 1,
            !bad,
            bad ? 'leaked reasoning / think tokens' : 'clean output',
          );
        },
      };
    },
    ...
    'number-fidelity': () => {
      return {
        ...
        grade: ({ output, testCase }) => {
          ...
          const nums = (output.text.match(/\b\d[\d,]*(?:\.\d+)?\b/g) ?? []).map((x) =>
            x.replace(/,/g, ''),
          );
          ...
          return det(
            ...
            nums.length === 0
              ? 'no numbers'
              : passed
                ? `all ${nums.length} numbers grounded`
                : `ungrounded: ${bad.slice(0, 3).join(', ')}`,
          );
        },
      };
    },
    ...
    compression: (spec) => {
      ...
      return {
        ...
        grade: ({ output, testCase }) => {
          ...
          return det(
            id,
            passed ? 1 : Math.max(0, maxRatio / ratio),
            passed,
